...
## 当てはまらない場面を skill 自身が持つ

公開 repo の README のように**読者がコードを読まない**文書は外部へのインターフェースなので、同じ事実を書いてよい。持ち主が無く、作れないなら、散文が唯一の持ち主なので消さない。この2つを skill 自身が言うので、当てはまらない範囲に無理に当たらない。
